...
//   const rawUrl = apiResult.annotated_video?.cloudinary_url || "";

//   const playableUrl = rawUrl
//     ? rawUrl.replace("/upload/", "/upload/f_mp4,vc_h264/")
//     : "";
...
//     annotated_video_url: playableUrl,
//     raw_annotated_video_url: rawUrl,
...
//     video_url: result.raw_annotated_video_url || result.annotated_video_url || "",
//     annotated_video_url:
//       result.raw_annotated_video_url || result.annotated_video_url || "",

//     pothole_count: result.pothole_count || 0,
//     severity: result.severity || "None",
//     damage_percentage: result.max_damage_percentage || 0,
...
//     frames_with_potholes: result.frames_with_potholes || 0,
//     total_frames_checked: result.total_frames_checked || 0,
//     total_frames_in_video: result.total_frames_in_video || 0,
//     fps: result.fps || 0,
...
//             <p className="text-muted mt-2">YOLO is analyzing video frames.</p>
//           </div>
//         )}
...
//                 <div className="bg-secondary border border-borderline rounded-3xl p-5">
//                   <h3 className="text-lg font-bold mb-4">Inspection Details</h3>
...
//                 <div className="bg-secondary border border-borderline rounded-3xl p-5">
//                   <h3 className="text-lg font-bold mb-4">Cloudinary Links</h3>
...
export default function VideoFlow() {
  ...
  const normalizeVideoResult = (apiResult) => {
    ...
    return {
      ...
      annotated_video_url: playableUrl,
      raw_annotated_video_url: rawUrl,
      cloudinary_public_id: apiResult.annotated_video?.public_id || "",
      cloudinary_upload_success:
        apiResult.annotated_video?.upload_success || false,
  ...
  const saveVideoReportToAdmin = async (result) => {
    const payload = {
      ...
      total_frames_in_video: result.total_frames_in_video || 0,
      fps: result.fps || 0,
      detections_by_frame: (result.pothole_frames || []).map((f) => ({
        frame: f.frame,
        potholes: f.detections?.length || 0,
      })),
    };

  

    const res = await axios.post(`${ADMIN_API}/api/reports`, payload, {
      headers: { "Content-Type": "application/json" },
    });
  ...
  const startProcessing = async () => {
    ...
    try {
      ...
      saveVideoReportToAdmin(result)
        .then(() => console.log("Video report saved to admin successfully"))
        .catch((saveError) => {
          console.error(
            "Admin video save failed:",
  ...
  const downloadSummary = () => {
    ...
    pdf.setFontSize(18);
    pdf.text("Video Pothole Detection Report", 14, 18);

    pdf.setFontSize(11);

    const rows = [
      ["Final Verdict", results.final_verdict],
      ["Pothole Found", results.has_pothole ? "Yes" : "No"],
      ["Total Boxes", results.pothole_count],
      ["Frames With Potholes", results.frames_with_potholes],
      ["Total Frames Checked", results.total_frames_checked],
      ["Total Frames In Video", results.total_frames_in_video],
      ["FPS", results.fps],
      ["Severity", results.severity],
      ["Damage Percentage", `${results.max_damage_percentage}%`],
      ["Traffic", metadata.traffic],
      ["Zone", metadata.zone],
      ["Camera Distance", metadata.distance ? `${metadata.distance} m` : "N/A"],
      ["Latitude", results.gps_location?.latitude || "N/A"],
      ["Longitude", results.gps_location?.longitude || "N/A"],
      ["Google Maps Link", results.gps_location?.google_maps_link || "N/A"],
  ...
  results.raw_annotated_video_url?.replace(
    "/upload/",
    "/upload/f_mp4,vc_h264/"
  ) || "";

pdf.text("Cloudinary Video:", 14, y);
y += 7;
...
y += 10;
    ...
    pdf.save(`Video_Pothole_Report_${Date.now()}.pdf`);
  };
              ...
              <div className="space-y-4">
                <div className="bg-secondary border border-borderline rounded-3xl p-5">
                  ...
                  <CleanRow
                    ...
                    value={
                      results.cloudinary_upload_success ? "Success" : "Failed"
                    }
                  />
                  ...
                  <CleanRow
                    ...
                    value={
                      results.raw_annotated_video_url ? "Available" : "Missing"
                    }
                  />
                </div>
